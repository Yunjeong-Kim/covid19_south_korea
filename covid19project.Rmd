...
```{r}
inputPanel(h6('03/22/2020'), style = "border-color: #00b4db" ) 
```
...
※ If respiratory symptoms appear and you suspicious of COVID-19, please get the consultation from 1339 call center or public health center first then visit a COVID-19 screening center.<br>
※ 出现呼吸道症状或疑似感染新型冠状病毒肺炎时，请先向管辖区保健所或1339呼叫中心进行咨询后再前往指定诊疗机构.</font>




Statistics {data-orientation=rows}
============================
...
pals <- colorFactor(c("#4169E1", "#CD5C5C"), domain = c(1, 0))
...
```{r}
renderValueBox({  valueBox(
                  value = format(round(death/confirmed, 2), nsmall=2),
                  icon = "fa-percent")})
```
...
Row {style="height:22pc;"}
-----------------------------------------------------------------------

### Daily Cases and Released

```{r}
colors1 <- c("Daily Confirmed" = "#CD5C5C", "Daily Released" = "#4682B4")
...
df <- data.frame(
  group = c("confirmed", "death"), 
  value = c(confirmed, death)
  )

df <- df %>%
  mutate(value = as.numeric((value))) %>%
  mutate(sum = sum(value)) %>%
  mutate(per = value/sum)

color4 <-  c('rgb(211,94,96)', 'rgb(128,133,133)', 'rgb(144,103,167)')
...
#plot_ly(df, labels=~group, values=~value, type = 'pie', 
 ...
 #       insidetextfont = list(color = '#FFFFFF'),
  #      hoverinfo = ~'text',
   #     text = ~paste(per, '% (', value, ')'), 
    #    marker = list(colors = colors), showlegend = TRUE)
```

### Total Cases by City
```{r}

totalCity <- data.frame(colSums(caseProvince[, 2:18]))
colnames(totalCity)[1] <- "cases"
totalCity <- tibble::rownames_to_column(totalCity, "city") 

plot_ly(totalCity, y=~city, x=~cases, type='bar', name='case') %>%
  layout(xaxis = list(title = 'Number of cases'))


```

Row {style="height:20pc;"}
-----------------------------------------------------------------------
...
```{r}
colors3 <- c("Seoul"="steelblue", "Daegu"="#CD5C5C", "Gyeonggi-do"="#00AFBB", "Gyeongsangbuk-do" = "#E7B800")
